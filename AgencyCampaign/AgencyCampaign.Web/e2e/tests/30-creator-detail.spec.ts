...
test.describe('Creator Detail - perfil 360 + redes sociais', () => {
  test('cria creator, abre 360, adiciona/edita/remove handle social', async ({ page, expectNoApiFailures }) => {
    ...
    // 7) cria handle
    await page.getByRole('button', { name: /Novo handle/i }).click()
    ...
    const handleRow = page.locator('[data-row="true"]', { hasText: `@e2e_${stamp}` }).first()
    ...
    // 9) deleta handle (botao Trash apos confirm)
    page.on('dialog', (d) => { void d.accept() })
    const updatedRow = page.locator('[data-row="true"]', { hasText: `@e2e_${stamp}_v2` }).first()
