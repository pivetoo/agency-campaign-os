...
import { PageLayout, useApi, usePermissions } from 'archon-ui'
...
export default function CommercialAnalytics() {
  ...
  return (
    <PageLayout title="Analytics do funil" subtitle="Conversão por estágio, ciclo de vendas e motivos de win/loss no período" showDefaultActions={false}>
        ...
        {analytics && (
          <>
            <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
              <KpiCard icon={<Award className="h-4 w-4 text-emerald-600" />} label="Fechadas no período" value={String(analytics.closedCount)} sub={`${analytics.wonCount} ganhas · ${analytics.lostCount} perdidas`} />
              <KpiCard icon={<Percent className="h-4 w-4 text-primary" />} label="Win rate" value={`${analytics.winRate.toFixed(1)}%`} sub={analytics.closedCount === 0 ? 'sem fechamentos' : 'ganhas / fechadas'} />
              <KpiCard icon={<Hourglass className="h-4 w-4 text-amber-600" />} label="Ciclo médio" value={`${analytics.averageCycleDays.toFixed(1)} dias`} sub="criação → fechamento" />
              <KpiCard icon={<Activity className="h-4 w-4 text-rose-600" />} label="Em andamento" value={String(analytics.conversionByStage.reduce((acc, s) => acc + s.stuck, 0))} sub="paradas no funil" />
            </div>

            <Section title="Conversão por estágio">
              <ConversionTable items={analytics.conversionByStage} />
            </Section>

            <div className="grid gap-5 lg:grid-cols-2">
              <Section title="Tempo médio por estágio">
            ...
            <div className="grid gap-5 lg:grid-cols-2">
              <Section title="Motivos de ganho" icon={<ThumbsUp className="h-4 w-4 text-emerald-600" />}>
                <ReasonsBlock items={analytics.winReasons} emptyLabel="Sem ganhos no período" />
              </Section>
              <Section title="Motivos de perda" icon={<ThumbsDown className="h-4 w-4 text-rose-600" />}>
                <ReasonsBlock items={analytics.lossReasons} emptyLabel="Sem perdas no período" />
              </Section>
            </div>
          </>
        )}
        ...
        return (
          <div key={item.stageId} className="rounded-md border border-border/60 bg-muted/20 px-3 py-2">
            <div className="flex items-baseline justify-between gap-2 text-sm">
              ...
              <span className="text-xs text-muted-foreground">{item.entered} entraram · <strong className="text-foreground">{item.conversionRate.toFixed(0)}%</strong> avançaram</span>
            </div>
            ...
            <div className="mt-1.5 grid grid-cols-3 gap-1 text-[10.5px]">
              <span className="text-emerald-700">avançaram {item.advanced} ({advancedPct.toFixed(0)}%)</span>
              <span className="text-amber-700">parados {item.stuck} ({stuckPct.toFixed(0)}%)</span>
              <span className="text-rose-700">perdidos {item.lost} ({lostPct.toFixed(0)}%)</span>
            </div>
          </div>
        )
      })}
        ...
        <div key={item.stageId}>
          <div className="flex items-baseline justify-between text-xs">
            ...
            <span className="text-muted-foreground">
              <strong className="text-foreground">{item.averageDays.toFixed(1)}</strong> dias · {item.samples} amostra{item.samples === 1 ? '' : 's'}
            </span>
          </div>
      ...
      {items.map((item, index) => (
        <div key={`${item.userId ?? 'none'}-${index}`} className="flex items-center gap-3 rounded-md border border-border/60 bg-muted/20 px-3 py-2">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[11px] text-muted-foreground">{item.wonCount} negócio{item.wonCount === 1 ? '' : 's'}</p>
          </div>
          ...
          return (
            <div key={`${item.reasonId ?? 'none'}-${item.reasonName}`} className="rounded-md border border-border/60 bg-muted/20 px-2.5 py-1.5">
              ...
              <div className="mt-1 flex items-center justify-between text-[11px] text-muted-foreground">
                <span>{share.toFixed(0)}% do valor</span>
