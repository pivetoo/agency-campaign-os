...
export default function CommercialAttention() {
  ...
  const goToAlert = (alert: CommercialAlert) => {
    navigate(`/comercial/oportunidades/${alert.opportunityId}${alert.followUpId ? '?tab=followups' : ''}`)
  }

  // Sem receita/valores no painel: so contagens e alertas acionaveis
  const chips: Chip[] = [
    ...
    { key: 'open', label: t('commercialAttention.chip.openOpportunities'), value: dashboard?.openOpportunities ?? 0, tone: 'muted', to: '/comercial/oportunidades' },
  ]
