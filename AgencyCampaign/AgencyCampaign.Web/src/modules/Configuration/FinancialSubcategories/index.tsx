...
export default function FinancialSubcategories() {
  ...
  const load = async () => {
    const result = await fetchAll(() => financialSubcategoryService.getAll({ page, pageSize, search: debouncedSearch || undefined, includeInactive: includeInactiveFilter === 'all' }))
  ...
  return (
    <>
      <PageLayout
        ...
        onRefresh={() => void load()}
        actionsSlot={<AuditUtilityBar entityName="FinancialSubcategory" entityLabel="Subcategoria financeira" entityId={selected?.id ?? null} />}
