...
namespace AgencyCampaign.Infrastructure.Services
{
    public sealed class CreatorPaymentService : CrudService<CreatorPayment>, ICreatorPaymentService
    {
        ...
        public async Task<CreatorPayment> MarkPaid(long id, MarkCreatorPaymentPaidRequest request, CancellationToken cancellationToken = default)
        {
            ...
            // Idempotencia (igual ao callback): so concilia/registra a saida se o pagamento ainda nao estava Pago.
            // Re-marcar um repasse ja pago nao pode dar baixa nem registrar saida de novo (dupla contagem).
            bool wasAlreadyPaid = payment.Status == PaymentStatus.Paid;
        ...
        private async Task SettlePlannedPayoutsAsync(CreatorPayment payment, DateTimeOffset paidAt, CancellationToken cancellationToken)
        {
            ...
            // #1: pagamento efetivado sem repasse previsto (criado manualmente ou entrega nunca publicada).
            // Sem registrar nada, o Pix sai mas o saldo/fluxo de caixa nao debitam - caixa inflado e risco de
            // pagar de novo. Registra a saida como um repasse JA PAGO na conta padrao (base bruta, igual aos previstos).
            if (planned.Count == 0)
            {
                long? outflowAccountId = await DbContext.Set<FinancialAccount>()
                    .AsNoTracking()
                ...
                if (!outflowAccountId.HasValue)
                {
                    payment.RegisterEvent(CreatorPaymentEventType.PlannedPayoutSettled,
                        $"Pagamento de R$ {payment.NetAmount:0.00} efetivado sem conta financeira ativa - nao foi possivel registrar a saida no caixa.");
                    return;
                }

                FinancialEntry outflow = new(
                    outflowAccountId.Value,
                    FinancialEntryType.Payable,
                    FinancialEntryCategory.CreatorPayout,
                    $"Repasse creator (pago sem previsto) #{payment.Id}",
                    payment.GrossAmount,
                    paidAt,
                    paidAt,
                    paymentMethod: payment.Method.ToString(),
                    referenceCode: null,
                    counterpartyName: payment.Creator?.StageName ?? payment.Creator?.Name,
                    notes: "Gerado ao marcar o repasse como pago sem repasse previsto correspondente.",
                    campaignId: campaignId,
                    campaignDeliverableId: null);
                outflow.LinkToCreator(payment.CreatorId);
                outflow.ChangeStatus(FinancialEntryStatus.Paid, paidAt);
                outflow.MarkAsAutoGenerated();
                DbContext.Set<FinancialEntry>().Add(outflow);

                payment.RegisterEvent(CreatorPaymentEventType.PlannedPayoutSettled,
                    $"Sem repasse previsto: registrada saida de R$ {payment.GrossAmount:0.00} no caixa para refletir o pagamento efetivado.");
                return;
            }
            ...
            // O budget usa o BRUTO (mesma base dos previstos = creatorAmount): a retencao reduz o liquido pago
            // mas nao a obrigacao, entao casar contra o liquido deixaria o previsto orfao (repasse fantasma).
            List<FinancialEntry> ordered = planned.OrderBy(item => item.DueAt).ToList();
            decimal remaining = payment.GrossAmount;
