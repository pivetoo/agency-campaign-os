...
namespace AgencyCampaign.Testing.Infrastructure.Services
{
    ...
    public sealed class OpportunityServiceTests
    {
        ...
        [Test]
        public async Task CloseAsWon_should_use_explicit_closed_value_when_provided()
        {
            await SeedBaseAsync();
            Opportunity opp = await service.CreateOpportunity(new CreateOpportunityRequest { BrandId = 1, Name = "Deal", EstimatedValue = 500m });
            Proposal proposal = new(opp.Id, "P", 1);
            ...
            await service.CloseAsWon(opp.Id, new CloseOpportunityAsWonRequest { WonNotes = "ok", ClosedValue = 999m });

            Opportunity? reloaded = await service.GetOpportunityById(opp.Id);
            reloaded!.ClosedValue.Should().Be(999m);
        }
