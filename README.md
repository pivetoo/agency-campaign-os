...
- A probabilidade é definida automaticamente pelo estágio, exceto quando o usuário a fixa manualmente (`SetProbability`/`ResetProbability`).
...
- **Oportunidades** (`/opportunities`): listar, `mine`, detalhe, `board`, `forecast`, `analytics`, `insights`; criar/atualizar; `ChangeStage`, `CloseAsWon`, `CloseAsLost`, `SetSource`, `SetProbability`, `ResetProbability`, `ReplaceTags`.
- **Propostas** (`/proposals`): listar, detalhe, `pdf`; criar/atualizar/excluir; itens (criar/editar/remover); `SendByEmail`, `SendByWhatsapp`, `MarkAsSent`, `MarkAsViewed`, `Approve`, `Reject`, `ConvertToCampaign`, `ConvertToNewCampaign`, `Cancel`; `ShareLinks` (criar/revogar) e `Versions`.
- **Proposta pública** (`/public/proposals/{token}` e `/pdf`): acesso por token, registra visualização.
- **Aprovações** (`/opportunityApprovals`): listar, `summary`, por negociação; criar; `Approve`, `Reject`, `MarkInReview`, `RequestChanges`, `Resubmit`; decisão de revisor; `Comments`, `Diffs`, `Impacts`.
